...
class Transformer(nn.Module):
    ...
    def __init__(self, 
        input_size: int = 5,
        dec_sequence_len: int = 21,
        batch_first: bool = True,
        out_sequence_len: int=7,
        dim_val: int=64,  
        ...
        Args:
            ...
            dec_sequence_len: int, the length of the input sequence fed to the decoder
        ...
        self.dec_sequence_len = dec_sequence_len
    ...
    def forward(self, src: Tensor, tgt: Tensor, src_mask: Tensor=None, 
        ...
        src = self.encoder( # src shape: [batch_size, enc_sequence_len, dim_val]
            src=src
            )
        #print("From model.forward(): Size of src after encoder: {}".format(src.size()))

        # Pass decoder input through decoder input layer
        #print("Size of tgt to go into decoder input layer: {}:".format(tgt.size()))
        N = tgt.size()[0] #batch size to fix reshape for some reason
        T = tgt.size()[1] #predict length to fix reshape for some reason
        decoder_output = self.decoder_input_layer(torch.reshape(tgt, (N,T,1))) # src shape: [target sequence length, batch_size, dim_val] regardless of number of input features
        ...
        return torch.reshape(decoder_output,(N,T))
...
class DLinear(nn.Module):
    ...
    def __init__(self, kernel_size=21, individual=False, sequence_len=21, pred_len=7, input_size=5):
        ...
        if self.individual:
            ...
            for i in range(self.channels):
                self.Linear_Seasonal.append(nn.Linear(self.sequence_len,self.pred_len))
                self.Linear_Trend.append(nn.Linear(self.sequence_len,self.pred_len))
            ...
            # self.Linear_Trend.weight = nn.Parameter((1/self.sequence_len)*torch.ones([self.pred_len,self.sequence_len]))

        self.Linear_output = nn.Linear(self.channels, 1)
    ...
    def forward(self, x):
        ...
        return x.reshape(x.size(0), self.pred_len) #.permute(0,2,1) # to [Batch, Output length, Channel]